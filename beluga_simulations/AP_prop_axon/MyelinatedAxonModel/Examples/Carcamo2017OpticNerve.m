% Replicate the optic nerve axon conduction velocity values from:
...
%%%%%%  OPTIC NERVE MYELINATED AXON SIMULATIONS %%%%%%%%%%%%%%%%%%%%%%%%%

% Directory of this file
thisDirectory                               = fileparts(mfilename('fullpath'));

saveDirectory                               = fullfile(pwd, 'Carcamo2017Results');
if ~isdir(saveDirectory)
    mkdir(saveDirectory)
end

defaultNodeLength                           = 1.02;
longNodeLength                              = 2.2;
peakNodeLength                              = 1.7;
shortNodeLength                             = 0.5;
matchCVNodeLength                           = 0.625;
gAct                                        = [30, 0.05, 0.8];
paranodePeriaxonalSpace                     = 0.007719692;
naDensityReductionPC                        = 26;
inodeLengthIncreasePC                       = 36;
PnodeLength                                 = 2.11;
...
% Carcamo2017OpticNerveAxon.m file, but reset here for clarity).
par.node.geo.length.value.vec(:)            = defaultNodeLength;

% Run the model.
fprintf('RUNNING MODEL 1 OF 9\n')
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerve.mat'));
...
% Set the lengths of the nodes close to the length giving a
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNervePeakNodeChanDensityConst.mat'));
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveShortNodeChanDensityConst.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveLongNodeChanConst.mat'));
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveShortNodeChanConst.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
par.myel.geo.peri.value.vec(:, [1, end])    = paranodePeriaxonalSpace * par.myel.geo.numlamellae.value.ref / (par.myel.geo.numlamellae.value.ref - 1);
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveNWrapsReduced.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Reduce the Na channel density by 26%
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveNaDensityReduced.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Increase internode length by 36% 
...
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveInternodeLengthIncreased.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Reset the length of the node (to 0.625um)
par.node.geo.length.value.vec(:)            = matchCVNodeLength;
par.node.seg.geo.length.value.vec           = repmat(par.node.geo.length.value.vec / par.geo.nnodeseg, 1, par.geo.nnodeseg);

% Run the model with updated parameters.
fprintf('RUNNING MODEL 9 OF 9\n')
Model(par, fullfile(saveDirectory, 'Carcamo2017OpticNerveCVMatchNodeLength.mat'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load(fullfile(saveDirectory, 'Carcamo2017OpticNerve.mat'))
vel1 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve CV:  %.2f m/s\n', vel1);

load(fullfile(saveDirectory, 'Carcamo2017OpticNerveLongNodeChanConst.mat'))
vel2 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve (node length 2.2um) CV:  %.2f m/s\n', vel2);
fprintf('       %.1f %% decrease\n', 100*(vel1-vel2)/vel1);

load(fullfile(saveDirectory, 'Carcamo2017OpticNerveShortNodeChanConst.mat'))
vel3 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve (node length 0.5um) CV:  %.2f m/s\n', vel3);
fprintf('       %.1f %% increase\n', 100*(vel3-vel1)/vel1);
fprintf('       %.1f %% increase on long node\n', 100*(vel3-vel2)/vel2);

load(fullfile(saveDirectory, 'Carcamo2017OpticNerveShortNodeChanDensityConst.mat'))
vel4 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve (node length 0.5um, channel density constant) CV:  %.2f m/s\n', vel4);
fprintf('       %.1f %% decrease\n', 100*(vel1-vel4)/vel1);

load(fullfile(saveDirectory, 'Carcamo2017OpticNervePeakNodeChanDensityConst.mat'))
vel5 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve (node length 1.7um, channel density constant) CV:  %.2f m/s\n', vel5);
fprintf('       %.1f %% increase\n', 100*(vel5-vel1)/vel1);
fprintf('       %.1f %% increase on short node\n', 100*(vel5-vel4)/vel4);

load(fullfile(saveDirectory, 'Carcamo2017OpticNerveNWrapsReduced.mat'))
vel6 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
fprintf('Optic nerve (6 myelin wraps) CV:  %.2f m/s\n', vel6);
fprintf('       %.1f %% decrease\n', 100*(vel1-vel6)/vel1);


load(fullfile(saveDirectory, 'Carcamo2017OpticNerveNaDensityReduced.mat'))
vel7 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
load(fullfile(saveDirectory, 'Carcamo2017OpticNerveInternodeLengthIncreased.mat'))
vel8 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);
load(fullfile(saveDirectory, 'Carcamo2017OpticNerveCVMatchNodeLength.mat'))
vel9 = velocities(MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR(2)-TIME_VECTOR(1), [20, 30]);

fprintf('\n\n\n')
fprintf('Optic nerve (6 myelin wraps) CV:                  %.2f m/s\n', vel6);
fprintf('Optic nerve (Na density reduced 26%%) CV:         %.2f m/s\n', vel7);
fprintf('Optic nerve (internode length increase 36%%) CV:  %.2f m/s\n', vel8);
fprintf('Optic nerve (node length 0.625um) CV:             %.2f m/s\n', vel9);