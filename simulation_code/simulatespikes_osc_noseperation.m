function [ids,ts,ei] = simulatespikes_osc(N,tmax,network,oscillation_freq)
    ...
    [ids,ts,ei,elevation,azimuth,parents] = simulatespikes(N,tmax,network.branchNo,oscillation_freq);
...
function [ids,ts,ei,elevation,azimuth,parents] = simulatespikes(N,tmax,branchNo,f0)
    ...
    N_ex_pre = ceil(N_ex_syn);
    % N_in_pre = ceil(N_in_syn/13.9);
    N_in_pre = ceil(N_in_syn);
    ...
    for i = 1:N_in_pre
        ...
        for j = 1:k
            ...
            t0 = tsE(idcs).*idcs2+interp1(oscDist,t,rand(nTrans,1),'nearest','extrap').*(1-idcs2);
            if(isnan(t0))
                oscDist
            end
