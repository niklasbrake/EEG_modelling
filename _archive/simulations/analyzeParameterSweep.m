...
for i = 1:length(F)
    if(exist(fullfile(folder,F(i).name,'power_spectra.mat'))~=0)
        continue;
    end
    folder2 = fullfile(folder,F(i).name,'LFPy');
    ...
    if(any(isnan(data.dipoles(:))))
        continue;
    end
    for j = 1:size(data.dipoles,3);
        temp = resample(data.dipoles(2:end,:,j),2e3,16e3);
        if(j==1)
    ...
    try
        [f,P] = expectedEEGspectrum(dp,1:size(dp,3),sa,idcs);
        save(fullfile(folder,F(i).name,'power_spectra.mat'),'f','P','tau','dtau','dgam','dlam');
    catch err
        disp(err.message);
    end
end
