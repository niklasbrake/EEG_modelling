...
##
## 2.   Decrease connections per neuron (3.6 syns/econ and 13.9 syns/icon)
...
with open(spikingFile, newline='') as csvfile:
     ...
     for row in spikeTimes:
        temp = row[0].split(',')
        if(temp[0]=='e'):
            eSpikes.append([int(x) for x in temp[1:]])
        elif(temp[0]=='i'):
            iSpikes.append([int(x) for x in temp[1:]])
...
for rep in range(nReps):
    ...
    while len(idcs)>0:
        ...
        idcs = np.argwhere(cell.get_idx_name(synE)[:,1]=='soma')
    # Generate presyanptic spike trains
    for i in range(nE/eSynsPerCon):
        m = np.random.poisson(tmax*eSyn['lambda'])
        preSpikeTrains.append(tmax*np.random.random(m))
    for i in range(nE):
        ...
        ts = np.random.choice(ts,len(ts)*success_rate,replace=False).tolist() # failure rate of 70% for spike to activate syanpse
    ...
    # Generate presyanptic spike trains
    for i in range(nI/iSynsPerCon):
        m = np.random.poisson(tmax*iSyn['lambda'])
        preSpikeTrains.append(tmax*np.random.random(m))
    for i in range(nI):
