...
load('E:\Research_Projects\004_Propofol\data\simulations\raw\passive\dipoles.mat');
tic;
[f,P] = expectedEEGspectrum(dp,1:size(dp,3),sa,1:100);
time = toc;
save('E:\Research_Projects\004_Propofol\data\simulations\raw\passive\PSD.mat','f','P','time');

function [f,P] = expectedEEGspectrum(dipoles,neuronIDs,sa,idcs)
    for j = 1:length(neuronIDs);
        temp = resample(dipoles(2:end,:,neuronIDs(j)),2e3,16e3);
        if(j==1)
