...
for i = 1:length(F)
    ...
    k0 = 1;
    dp = zeros(7400,3,2*length(F));
    for j = 1:length(F2)
        folder3 = fullfile(folder2,F2(j).name,'s=0','LFPy');
        F3 = dir(folder3); F3 = F3(3:end);
        for k = 1:length(F3)
            data = csvread(fullfile(folder3,F3(k).name),1,0);
            dp(:,:,k0) = resample(data(0.2*16e3+1:end-0.2*16e3-1,2:4),2e3,16e3);
            k0 = k0+1;
        end
    end
    [f,P(:,:,i)] = expectedEEGspectrum(dp,1:size(dp,3),sa,1e3);
end

save(fullfile(folder,'power_spectra.mat'),'f','P');

function [f,P] = expectedEEGspectrum(dipoles,neuronIDs,sa,idcs)
    q = gpuArray(dipoles);
    for j = 1:length(idcs)
        eeg = getEEG(q,sa,idcs(j));
        temp = mypmtm(detrend(eeg),2e3,2);
       if(j==1)
            P = zeros(size(temp));
        end
        P = P+temp;
