...
iSyn = {'tau1':1,
        'tau2':2*10*(1+propofol), # 10 ms
        'weight':0.001*(1+propofol), # uS (1.4 nS) [Zsiros and Hestrin. J. Neurophysiol. 2005] (10-30 pS)
        'erev':-80,
...
for rep in range(nReps):
    np.random.seed()
    cell.append(LFPy.Cell(sectionList,v_init=eLeak,celsius=37))
    cell[rep].tstop = tmax
    syn = list()
    synIds = list()
    ei = list()
    ...
    synE = cell[rep].get_rand_idx_area_norm(section='allsec', nidx=nE, z_min=- 1000000.0, z_max=1000000.0)
    ...
    for i in range(nE):
        ...
        syn.append(LFPy.Synapse(cell[rep], **synParams))
    ...
    synI = cell[rep].get_rand_idx_area_norm(section='allsec', nidx=nI, z_min=-1000, z_max=10000)
    np.random.seed(10)
    # set spike times for every synapse
    for i in range(nI):
        mI = np.random.poisson(lambdaI*tmax/1000) # total number events
...
# simulate_model(cell[0])
pool = mp.Pool(mp.cpu_count())
results = pool.map_async(simulate_model,cell)
nt.bell()